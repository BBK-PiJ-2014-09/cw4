...
//	10	The list contains a minimum of one contact (if there were just two people: the user and the contact) and may contain an arbitrary number of them.
	...
	public void testAddNullMeetingNotes() 
	{
		...
		Calendar cal = new GregorianCalendar(2015,Calendar.JANUARY,21,9,0);
	...
	public void testGetContactsByString() 
	{
		...
		actualContactSet = manager.getContacts("J");
		assertEquals(expectedContactSet, actualContactSet);
	}

	@Test(expected = NullPointerException.class)
	public void testGetContactsWithNullString() 
	{
		//test throws NullPointerException if the parameter is null
		//test 48
		ContactManagerImpl manager = new ContactManagerImpl();
		manager.addNewContact("Johnny", "VIP");
		manager.addNewContact("Jane", "High net worth");
	...
	@Test(expected = NullPointerException.class)
	public void testGetContactsByNullIDs() 
	{
		//test throws NullPointerException if the parameter is null
		//test 49
		ContactManagerImpl manager = new ContactManagerImpl();
